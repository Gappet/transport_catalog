...
SphereProjector CreatorSphereProjector(
    std::deque<Stop>& stops, RenderSettings& settings) {
...
//	continue;
//    }
    if (!bus.route_stops_.empty() || bus.route_stops_.size() == 1) {
      ...
      if (bus.is_roundtrip) {
        ...
        text_first_secon_stop
            .SetPosition(projector(
                bus.route_stops_[(bus.route_stops_.size() / 2)]->coordinates_))
            .SetOffset(settings.bus_label_offset)
            .SetFontSize(settings.bus_label_front_size)
            .SetFontFamily("Verdana"s)
            .SetFontWeight("bold")
...
std::vector<svg::Circle> DrawStop(
    const std::deque<Stop>& stops, RenderSettings& settings,
    SphereProjector& projector) {
...
json::Node GetMapOfRoad(::transpot_guide::TransportCatalogue& transport_catalog,
  ...
  SphereProjector projector =
      CreatorSphereProjector(stops, settings);
  ...
  std::vector<svg::Circle> stop_points =
      DrawStop(stops, settings, projector);
